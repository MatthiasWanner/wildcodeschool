
  /*

    🏁 Please take this quick challenge and rename carefully all the variable, and fix this broken code by assigning the correct 
    variable keyword you have learned in the quest.
    ex: const name = "David";

  */

    /*
    a = "David";
    b = "Bruno";
    z = "Hi, I'm David Bruno from SF, I like to cook and meet new people.";
  
    1img = "http://www.go.com/davif.png";
  
    lll = "San Francisco";
    abstract = "CA";
  
    ppl_follower = 109;
    people_following = 1500;
  
  
    WHATTIMEisIT = new Date();
  
    /*
  
      🏁 David is following one more account, increment the total of following account
  
    */
    

    // Solution
    const firstName = "David";
    const lastName = "Bruno";
    let message = "Hi, I'm " + firstName + " " + lastName + " from SF, I like to cook and meet new people.";

    let image1 = "http://www.go.com/davif.png";

    const city = "San Francisco";
    const state = "CA";

    let followers = 109;
    let peopleFollowing = 1500;

    console.log("Le nombre de gens suivi est de :" + peopleFollowing + "!");

    WHATTIMEisIT = new Date();

    /*

        🏁 David is following one more account, increment the total of following account

    */

    peopleFollowing ++;
    console.log("Le nombre de gens suivi est de :" + peopleFollowing + "!");

  
  